...
LB = {
 ...
 '千葉':('rm',-14,-14,1),'蘇我':('lb',8,-12,1),'誉田':('mt',4,14,1),'大網':('lm',14,0,1),'茂原':('lm',14,0,0),
...
def uchibo_pts():
    ...
    蘇我の駅名は東側（右上）に置き、この赤線を隠さないようにしている。"""
...
item('purple',['終日運転取りやめ','（再開の見通しは未定。','　千葉〜誉田は24日始発','　から再開見込み）'])
item('red',['本数を減らして運転','（23日始発から運転再開）'])
item('thin',['細線：計画の発表なし'])
